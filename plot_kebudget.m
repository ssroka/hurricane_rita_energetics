...
KE_bug_material = KE_bug_dqdt - KE_bug_adv;

KE_bug_totpress = KE_bug_dpdr+KE_bug_dpdz;

keyboard
    
            figure(5)
            [c,h] = contourf(raddis,zc1,KE_bug_material',[-1:0.1:1]);
            set(h,'edgecolor','none')
            editFig(1,x_bnds,y_bnds)
            title('Material Derivative of Resolved-Scale KE (m^2 s^-^2 s^-^1)')

            figure(6)
            [c,h] = contourf(raddis,zc1,KE_bug_totpress',[-1:0.1:1]);
            set(h,'edgecolor','none')
            editFig(1,x_bnds,y_bnds)
            title('Material Derivative of Resolved-Scale KE (m^2 s^-^2 s^-^1)')

            figure(7)
            [c,h] = contourf(raddis,zc1,KE_bug_g',[-1:0.1:1]);
            set(h,'edgecolor','none')
            editFig(1,x_bnds,y_bnds)
            title('Material Derivative of Resolved-Scale KE (m^2 s^-^2 s^-^1)')

            figure(8)
            [c,h] = contourf(raddis,zc1,KE_bug_tot_P',[-1:0.1:1]);
            set(h,'edgecolor','none')
            editFig(1,x_bnds,y_bnds)
            title('Material Derivative of Resolved-Scale KE (m^2 s^-^2 s^-^1)')

            figure(9)
            [c,h] = contourf(raddis,zc1,KE_bug_uTau',[-1:0.1:1]);
            set(h,'edgecolor','none')
            editFig(1,x_bnds,y_bnds)
            title('SFS Transport of Resolved-Scale KE (m^2 s^-^2 s^-^1)')
