...
utau(:,k,1,1) = ddr(fradl.*Tau(:,k,1,1),DR);
utau(:,k,1,2) = 0.0;
utau(:,k,1,3) = ddz(fradl.*Tau(:,k,1,3),...
                    fradl_b.*Tau(:,k_ind_b,1,3),...
                    fradl_t.*Tau(:,k_ind_t,1,3),DZ,T_flag,B_flag);

utau(:,k,2,1) = ddr(ftang.*Tau(:,k,2,1),DR);
utau(:,k,2,2) = 0.0;
utau(:,k,2,3) = ddz(ftang.*Tau(:,k,2,3),...
                    ftang_b.*Tau(:,k_ind_b,2,3),...
                    ftang_t.*Tau(:,k_ind_t,2,3),DZ,T_flag,B_flag);

utau(:,k,3,1) = ddr(fvert.*Tau(:,k,3,1),DR);
utau(:,k,3,2) = 0.0;
utau(:,k,3,3) = ddz(fvert.*Tau(:,k,3,3),...
                    fvert_b.*Tau(:,k_ind_b,3,3),...
                    fvert_t.*Tau(:,k_ind_t,3,3),DZ,T_flag,B_flag);
