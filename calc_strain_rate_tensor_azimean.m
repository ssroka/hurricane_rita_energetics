...
advection(:,k,1,1) = ddr(fradl.*fradl.*fradl,DR);
advection(:,k,1,2) = 0.0;
advection(:,k,1,3) = ddz(fradl.*fradl.*fvert,...
                         fradl_b.*fradl_b.*fvert_b,...
                         fradl_t.*fradl_t.*fvert_t,DZ,T_flag,B_flag);
advection(:,k,2,1) = ddr(ftang.*ftang.*fradl,DR);
advection(:,k,2,2) = 0.0;
advection(:,k,2,3) = ddz(ftang.*ftang.*fvert,...
                         ftang_b.*ftang_b.*fvert_b,...
                         ftang_t.*ftang_t.*fvert_t,DZ,T_flag,B_flag);
advection(:,k,3,1) = ddr(fvert.*fvert.*fradl,DR);
advection(:,k,3,2) = 0.0;
advection(:,k,3,3) = ddz(fvert.*fvert.*fvert,...
                         fvert_b.*fvert_b.*fvert_b,...
                         fvert_t.*fvert_t.*fvert_t,DZ,T_flag,B_flag);


utau(:,k,1,1) = ddr(-2.*fradl.*Tau(:,k,1,1),DR);
utau(:,k,1,2) = 0.0;
utau(:,k,1,3) = ddz(-2.*fradl.*Tau(:,k,1,3),...
                    -2.*fradl_b.*Tau(:,k_ind_b,1,3),...
                    -2.*fradl_t.*Tau(:,k_ind_t,1,3),DZ,T_flag,B_flag);

utau(:,k,2,1) = ddr(-2.*ftang.*Tau(:,k,2,1),DR);
utau(:,k,2,2) = 0.0;
utau(:,k,2,3) = ddz(-2.*ftang.*Tau(:,k,2,3),...
                    -2.*ftang_b.*Tau(:,k_ind_b,2,3),...
                    -2.*ftang_t.*Tau(:,k_ind_t,2,3),DZ,T_flag,B_flag);

utau(:,k,3,1) = ddr(-2.*fvert.*Tau(:,k,3,1),DR);
utau(:,k,3,2) = 0.0;
utau(:,k,3,3) = ddz(-2.*fvert.*Tau(:,k,3,3),...
                    -2.*fvert_b.*Tau(:,k_ind_b,3,3),...
                    -2.*fvert_t.*Tau(:,k_ind_t,3,3),DZ,T_flag,B_flag);
