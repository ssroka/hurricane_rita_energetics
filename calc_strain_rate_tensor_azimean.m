...
utau(:,k,1,2) = 0.0;
utau(:,k,1,3) = ddz(Tau(:,k,1,3),...
                    Tau(:,k_ind_b,1,3),...
                    Tau(:,k_ind_t,1,3),DZ,T_flag,B_flag).*fradl;

utau(:,k,2,1) = ddr(Tau(:,k,2,1),DR).*ftang;
utau(:,k,2,2) = 0.0;
utau(:,k,2,3) = ddz(Tau(:,k,2,3),...
                    Tau(:,k_ind_b,2,3),...
                    Tau(:,k_ind_t,2,3),DZ,T_flag,B_flag).*ftang;

utau(:,k,3,1) = ddr(Tau(:,k,3,1),DR).*fvert;
utau(:,k,3,2) = 0.0;
utau(:,k,3,3) = ddz(Tau(:,k,3,3),...
                    Tau(:,k_ind_b,3,3),...
                    Tau(:,k_ind_t,3,3),DZ,T_flag,B_flag).*fvert;
